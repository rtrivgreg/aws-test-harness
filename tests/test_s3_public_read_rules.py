"""S3_BUCKET_PUBLIC_READ_PROHIBITED — public GetObject policy on/off."""
...
def _spec() -> ManagedRuleSpec:
    return ManagedRuleSpec(
        ...
        toggle_strategy="s3_public_read",
    )


@pytest.mark.s3
@pytest.mark.slow
def test_s3_bucket_public_read_prohibited(
    s3_test_bucket: str,
    ...
    try:
        ...
        s3_toggle.make_public_read_noncompliant()
        change_ts = time.time()
        ...
        s3_toggle.make_public_read_compliant()
        change_ts = time.time()
        compliance.wait_for_config_item_after(
            resource_id=s3_test_bucket,
            after_timestamp=change_ts,
        ...
        log(f"{spec.rule_name} passed public-read cycle", style="green")
    finally:
        try:
            s3_toggle.make_public_read_compliant()
