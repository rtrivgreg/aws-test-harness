"""S3_BUCKET_TAGGED — required key HarnessProof present vs absent."""
...
def _spec() -> ManagedRuleSpec:
    return ManagedRuleSpec(
        ...
        source_identifier="S3_BUCKET_TAGGED",
        input_parameters={"requiredKeyTags": "HarnessProof"},
        resource_types=["AWS::S3::Bucket"],
        toggle_strategy="s3_tagged",
    )


@pytest.mark.s3
@pytest.mark.slow
def test_s3_bucket_tagged(
    s3_test_bucket: str,
    ...
    try:
        ...
        s3_toggle.make_tagged_noncompliant()
        change_ts = time.time()
        ...
        s3_toggle.make_tagged_compliant()
        change_ts = time.time()
        compliance.wait_for_config_item_after(
            resource_id=s3_test_bucket,
            after_timestamp=change_ts,
        ...
        log(f"{spec.rule_name} passed tagged cycle", style="green")
    finally:
        try:
            s3_toggle.make_tagged_compliant()
