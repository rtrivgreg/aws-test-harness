"""EBS_SNAPSHOT_BLOCK_PUBLIC_ACCESS — unblocked NC, block-all-sharing C.

Change-triggered account/region setting. Restore block-all in finally.
Config resourceId is the account id.
"""
...
from harness.ebs_bpa_toggle import SnapshotBpaToggle


def _spec() -> ManagedRuleSpec:
    return ManagedRuleSpec(
        rule_name="ebs-snapshot-block-public-access",
        source_identifier="EBS_SNAPSHOT_BLOCK_PUBLIC_ACCESS",
        input_parameters={},
        resource_types=["AWS::EC2::SnapshotBlockPublicAccess"],
        toggle_strategy="ebs_snapshot_bpa",
    )
...
@pytest.mark.ebs
@pytest.mark.slow
def test_ebs_snapshot_block_public_access(
    config_mgr: ConfigRuleManager,
    compliance: ComplianceChecker,
    aws_region: str,
) -> None:
    account = boto3.client("sts").get_caller_identity()["Account"]
    toggle = SnapshotBpaToggle(region=aws_region)
    spec = _spec()
    rule_name = None
    passed = False
    try:
        log(f"===== Testing rule: {spec.rule_name} account={account} =====")
        log(f"Current BPA state={toggle.state()}")
        compliance.wait_for_resource_discovered(
            resource_id=account,
            resource_type="AWS::EC2::SnapshotBlockPublicAccess",
            timeout_seconds=180,
        )
        rule_name = config_mgr.put_managed_rule(spec, resource_id=account)
        ...
        log(f"{spec.rule_name} passed BPA cycle", style="green")
        passed = True
    finally:
        try:
            toggle.enable_block_all()
        except Exception as exc:
            log(f"restore BPA block-all: {exc}", style="yellow")
