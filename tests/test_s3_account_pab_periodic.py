"""S3_ACCOUNT_LEVEL_PUBLIC_ACCESS_BLOCKS_PERIODIC — one flag off NC, all on C.

Periodic AWS::::Account. Restore all-four-true in finally.
"""
...
from harness.s3_account_pab import AccountPabToggle
...
def _spec() -> ManagedRuleSpec:
    return ManagedRuleSpec(
        ...
        toggle_strategy="s3_account_pab",
    )
...
@pytest.mark.s3
@pytest.mark.slow
def test_s3_account_level_public_access_blocks_periodic(
    config_mgr: ConfigRuleManager,
    compliance: ComplianceChecker,
    aws_region: str,
) -> None:
    account = boto3.client("sts").get_caller_identity()["Account"]
    toggle = AccountPabToggle(account=account, region=aws_region)
    spec = _spec()
    rule_name = None
    passed = False
    try:
        log(f"===== Testing rule: {spec.rule_name} account={account} =====")
        log(f"Current account PAB={toggle.get()}")
        ...
        log(f"{spec.rule_name} passed account PAB cycle", style="green")
        passed = True
    finally:
        try:
            toggle.all_on()
            log(f"Restored account PAB={toggle.get()}")
        except Exception as exc:
            log(f"restore account PAB: {exc}", style="yellow")
