"""S3_BUCKET_ACL_PROHIBITED — BucketOwnerEnforced vs ACLs enabled."""
...
def _spec() -> ManagedRuleSpec:
    return ManagedRuleSpec(
        ...
        toggle_strategy="s3_acl_prohibited",
    )


@pytest.mark.s3
@pytest.mark.slow
def test_s3_bucket_acl_prohibited(
    s3_test_bucket: str,
    ...
    try:
        ...
        s3_toggle.make_acl_noncompliant()
        change_ts = time.time()
        ...
        log(f"{spec.rule_name} passed ACL-prohibited cycle", style="green")
    finally:
        try:
            s3_toggle.make_acl_compliant()
