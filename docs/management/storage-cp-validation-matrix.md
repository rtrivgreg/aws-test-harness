...
Lab: `418295699841` `us-east-1` run `ef57dcf4` as of 2026-08-28.  
...
|--:|---|---|---|---|---|
| 1 | backup-plan-min-frequency-and-min-retention-check | BACKUP_PLAN_MIN_FREQUENCY_AND_MIN_RETENTION_CHECK | Backup | live | Harness Backup module |
...
| 12 | ebs-resources-protected-by-backup-plan | EBS_RESOURCES_PROTECTED_BY_BACKUP_PLAN | EBS | catalog-only | |
| 13 | ebs-snapshot-block-public-access | EBS_SNAPSHOT_BLOCK_PUBLIC_ACCESS | EBS | catalog-only | |
| 14 | ebs-snapshot-public-restorable-check | EBS_SNAPSHOT_PUBLIC_RESTORABLE_CHECK | EBS | partial | NC proven; C account-scoped xfail |
| 15 | ec2-ebs-encryption-by-default | EC2_EBS_ENCRYPTION_BY_DEFAULT | EBS | catalog-only | Account default; not the volume toggle |
| 16 | ec2-launch-templates-ebs-volume-encrypted | EC2_LAUNCH_TEMPLATES_EBS_VOLUME_ENCRYPTED | EBS | catalog-only | |
| 17 | ec2-spot-fleet-request-ct-encryption-at-rest | EC2_SPOT_FLEET_REQUEST_CT_ENCRYPTION_AT_REST | EBS | catalog-only | |
| 18 | efs-access-point-enforce-root-directory | EFS_ACCESS_POINT_ENFORCE_ROOT_DIRECTORY | EFS | live | Access-point fixture |
| 19 | efs-access-point-enforce-user-identity | EFS_ACCESS_POINT_ENFORCE_USER_IDENTITY | EFS | live | Access-point fixture |
...
| 22 | efs-filesystem-ct-encrypted | EFS_FILESYSTEM_CT_ENCRYPTED | EFS | catalog-only | |
| 23 | efs-in-backup-plan | EFS_IN_BACKUP_PLAN | EFS | catalog-only | |
| 24 | efs-mount-target-public-accessible | EFS_MOUNT_TARGET_PUBLIC_ACCESSIBLE | EFS | catalog-only | |
| 25 | efs-resources-protected-by-backup-plan | EFS_RESOURCES_PROTECTED_BY_BACKUP_PLAN | EFS | catalog-only | |
...
| 29 | fsx-openzfs-copy-tags-enabled | FSX_OPENZFS_COPY_TAGS_ENABLED | FSx | parked | FS AVAILABLE; Config CI never created |
...
| 34 | s3-account-level-public-access-blocks-periodic | S3_ACCOUNT_LEVEL_PUBLIC_ACCESS_BLOCKS_PERIODIC | S3 | catalog-only | Account-level periodic |
| 35 | s3-bucket-acl-prohibited | S3_BUCKET_ACL_PROHIBITED | S3 | live | 2026-08-28 |
...
| 38 | s3-bucket-default-lock-enabled | S3_BUCKET_DEFAULT_LOCK_ENABLED | S3 | catalog-only | |
...
| 47 | s3-bucket-server-side-encryption-enabled | S3_BUCKET_SERVER_SIDE_ENCRYPTION_ENABLED | S3 | parked | Implicit SSE-S3; NC unprovable |
| 48 | s3-bucket-ssl-requests-only | S3_BUCKET_SSL_REQUESTS_ONLY | S3 | live | |
| 49 | s3-bucket-tagged | S3_BUCKET_TAGGED | S3 | catalog-only | |
| 50 | s3-bucket-versioning-enabled | S3_BUCKET_VERSIONING_ENABLED | S3 | live | |
| 51 | s3-default-encryption-kms | S3_DEFAULT_ENCRYPTION_KMS | S3 | live | 2026-08-28 |
| 52 | s3-event-notifications-enabled | S3_EVENT_NOTIFICATIONS_ENABLED | S3 | catalog-only | |
| 53 | s3express-dir-bucket-lifecycle-rules-check | S3EXPRESS_DIR_BUCKET_LIFECYCLE_RULES_CHECK | S3 | catalog-only | Directory buckets |
...
| 58 | s3-resources-protected-by-backup-plan | S3_RESOURCES_PROTECTED_BY_BACKUP_PLAN | S3 | catalog-only | |
| 59 | s3-version-lifecycle-policy-check | S3_VERSION_LIFECYCLE_POLICY_CHECK | S3 | live | |

**Counts:** live 14 · partial 1 · parked 2 · catalog-only 42 · total 59.
